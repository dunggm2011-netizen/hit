...
# THÔNG TIN BOT
# =========================
TOKEN = os.environ.get("BOT_TOKEN", "8619392859:AAGah_Fb0upGal4sw7lw4aZVHR_6qYDqVMc")
...
def start(message):
    user_data.setdefault(message.chat.id, {})["step"] = "idle"
    bot.reply_to(
        message,
        ...
        "🎲 <b>Cơ chế cược:</b>\n"
        f"• X2 khi thua\n"
        f"• Gãy {MAX_LOSE_STREAK} tay → tạm dừng, chờ phiên AI dự đúng\n"
        "• Thắng → quay về vốn gốc\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "<i>Phiên bản: 7.1 - Martingale V3</i>"
    )
...
print(f"[{time.strftime('%H:%M:%S')}] 🤖 ENGINE AI CORE v7.1 (Flask + Martingale V3) đã bật.")
...
print(f"[{time.strftime('%H:%M:%S')}] 🌐 Flask health server PORT {os.environ.get('PORT', 5000)}")
